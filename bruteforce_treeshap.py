...
def compute_tree_shap(xgb_model, x, features, feature_names):
    ...
    :param features: set of feature we want to study
    ...
    for tree_index in range(num_trees):
        ...
        for current_feature in features:
            for subset in powerset(features - {current_feature}):
                w = 1/(binom(n_f-1,len(subset))*n_f)
                total_shap_value[current_feature] -= w*EXPVALUE(x, subset, tree)
                total_shap_value[current_feature] += w*EXPVALUE(x, subset + (current_feature), tree)

    
    return {feature_names[feat]:shap for feat,shap in total_shap_value.items()}

def compute_shap_for_dataset(xgb_model, X_train, features, nb_samples=None, precise_samples=None):
    """
